...
contract LVRAuctionHookBasicTest is Test {
    ...
    function testPriceInversionLogic() public view {
        // Test price inversion logic through the test contract
        Currency tokenA = Currency.wrap(address(0x1));
        Currency tokenB = Currency.wrap(address(0x2));
        
        bool shouldInvert = hook.shouldInvertPrice(tokenA, tokenB);
        bool reverseInvert = hook.shouldInvertPrice(tokenB, tokenA);
        
        // One should be true and the other false
        assertTrue(shouldInvert != reverseInvert);
    }
    
    function testSubmitAuctionResultUnauthorized() public {
