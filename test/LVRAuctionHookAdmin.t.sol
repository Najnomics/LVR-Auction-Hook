...
/**
 * @title LVRAuctionHook Admin Functions Tests
 * @notice Tests admin functions and auction management for complete coverage
 */
contract LVRAuctionHookAdminTest is Test {
    ...
    function test_SetLVRThreshold_TooHigh() public {
        vm.prank(owner);
        vm.expectRevert("LVRAuctionHook: threshold too high");
    ...
    function test_SetFeeRecipient_ZeroAddress() public {
        vm.prank(owner);
        vm.expectRevert("LVRAuctionHook: invalid address");
    ...
    function test_SubmitAuctionResult_UnauthorizedOperator() public {
        ...
        vm.prank(nonOwner);
        vm.expectRevert("LVRAuctionHook: unauthorized operator");
    ...
    function test_SubmitAuctionResult_InactiveAuction() public {
        ...
        vm.prank(operator);
        vm.expectRevert("LVRAuctionHook: auction not active");
    ...
    function test_SubmitAuctionResult_AuctionNotEnded() public {
        ...
        vm.prank(operator);
        vm.expectRevert("LVRAuctionHook: auction not ended");
        hook.submitAuctionResult(auctionId, address(0x777), 5 ether);
    }
    ...
    function test_ClaimRewards_NoLiquidity() public {
        vm.prank(user);
        vm.expectRevert("LVRAuctionHook: no liquidity provided");
