...
/**
 * @title LVRAuctionHook
 * @author LVR Auction Hook Team
 ...
 */
contract LVRAuctionHook is BaseHook, ReentrancyGuard, Ownable, Pausable {
    ...
    modifier onlyAuthorizedOperator() {
        require(authorizedOperators[msg.sender], "LVR Auction Hook: unauthorized operator");
        _;
    }
    
    modifier onlyActiveAuction(bytes32 auctionId) {
        require(auctions[auctionId].isActive, "LVR Auction Hook: auction not active");
        _;
    }
    ...
    function submitAuctionResult(
        ...
        require(
            ...
            "LVR Auction Hook: auction not ended"
        );
    ...
    function _updateLPRewards(PoolId poolId, uint256 rewardAmount) internal {
        // Accumulate rewards for proportional distribution to LPs
        // This will be distributed when LPs claim their rewards
    ...
    function claimRewards(PoolId poolId) external nonReentrant {
        ...
        require(userLiquidity > 0, "LVR Auction Hook: no liquidity provided");
    ...
    function setLVRThreshold(uint256 newThreshold) external onlyOwner {
        require(newThreshold <= 1000, "LVR Auction Hook: threshold too high"); // Max 10%
    ...
    function setFeeRecipient(address newFeeRecipient) external onlyOwner {
        require(newFeeRecipient != address(0), "LVR Auction Hook: invalid address");
